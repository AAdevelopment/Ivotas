...
public class Server_RMI  extends UnicastRemoteObject implements Comunication_server {
   ...
   @Override
    ...
    public void savePessoas(){
        try {
            FileWriter file= new FileWriter("C:\\Users\\Admin\\Desktop\\3_ano_1_sem\\SD\\Projecto1\\Ivotas\\src\\Pessoas.txt");
            BufferedWriter out = new BufferedWriter(file);
            out.write("nome;cartao;password;departamento;cartao_val;telemovel;morada");
            out.newLine();
            for(int i=0;i<this.Pessoas.size();i++){
                Pessoa aux= this.Pessoas.get(i);
                out.write(aux.name+aux.tipo_pessoa+aux.cartao+aux.Password+aux.morada+aux.tel+aux.card_valid+aux.Dpto);
                out.newLine();
            }
        } catch (IOException ex) {
            Logger.getLogger(Server_RMI.class.getName()).log(Level.SEVERE, null, ex);
